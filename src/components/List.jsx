...
const List = ({ lists, match }) => {
  ...
  function removeItem() {
    ...
    console.log("Remove the last item in the list: " + list.list_items[list.list_items.length-1].name);
  }

  function deleteList() {
    //Remove this list and reroute to home
    console.log("This will delete " + list.list_name);
  }
  ...
  return (
    <div className="list-viewer-container">
      <h1>{list.list_name}</h1>
        ...
        <br/>
        <input className="listBtn" onClick={addItem} type="button" value="Add Item"/>
        <input className="listBtn" onClick={removeItem} type="button" value="Remove Item"/>
        <input className="listBtn" onClick={deleteList} type="button" value="Delete List"/> <br/>
        <input className="listText" onChange={event => setText(event.target.value)} type="textField" value={text}/>
    </div>
  );
};
